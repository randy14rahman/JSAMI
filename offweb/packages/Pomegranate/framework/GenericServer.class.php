...
class GenericServer implements RpcServer\RpcInterface
{
    ...
    public function error_handler($errno, $errstr, $errfile, $errline, array $errcontext)
    {
        if ($this->response != null) {
            require_once __DIR__.'/RpcServer/Error.class.php';
            $errobj = new RpcServer\Error($errstr, $errno, $errfile . ' [' . $errline . ']');
            $this->response->setError($errobj);
        }
        else {
            $errstr = "$errstr($errno)\n $errfile [$errline]";
            echo $errstr;
