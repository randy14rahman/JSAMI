...
class Mapper
{
	...
	public function executeQuery(QueryInfo $query)
	{
		...
		try {
			...
			$stat = self::$dbConnection[$this->connectionIndex]->prepare($query->SQL);
			$stat->execute();
			$this->numberOfAffectedRows = $stat->rowCount();
			self::commit($key);
	...
	public function loadListByQuery(QueryInfo $query)
	{
		...
		try {
			$key = self::beginTransaction();
			$stmt = self::$dbConnection[$this->connectionIndex]->query($query->SQL);
		}
		catch (\Exception $e) {
			self::rollBack($key);
	...
	private static function toHierarchy(array $data, array $columnGroups, array $columnNames)
	{
		...
		foreach ($cgKeys as $cgKey) {
			...
			$columnGroup->extra['ids'] = array();
			$columnGroup->extra['parent_ids'] = array();
			foreach ($data as $row) {
				$id = $row[$columnGroup->extra['index']];
				if ($id != null && !isset($columnGroup->extra['ids'][$id])) {
					$information = new \stdClass();
					for ($i=$columnGroup->extra['from']; $i<=$columnGroup->extra['to']; $i++)
						$information->{$columnNames[$i]} = $row[$i];
					$columnGroup->extra['ids'][$id] = $information;
					unset($information);
				}

				if ($id != null) {
					if ($columnGroup->extra['parent'] != null) {
						$parent_id = $row[$columnGroup->extra['parent']->extra['index']];
						if (!isset($columnGroup->extra['parent_ids'][$parent_id][$id]))
							$columnGroup->extra['parent_ids'][$parent_id][$id] =& $columnGroup->extra['ids'][$id];
					}
				}
			}
		...
		foreach ($cgKeys as $cgKey) {
			...
			if ($columnGroup->extra['parent'] != null) {
				...
				$parent =& $columnGroup->extra['parent']->extra['ids'];
				foreach ($columnGroup->extra['parent_ids'] as $parentid => $children) {
		...
		if ($columnGroups[$cgKey]->extra['parent'] != null) {
			$count1 = count($columnGroups[$cgKey]->extra['parent']->extra['ids']);
			$cgKeys1 = array_keys($columnGroups[$cgKey]->extra['parent']->extra['ids']);
			$count2 = count($columnGroups[$cgKey]->container);
			$cgKeys2 = array_keys($columnGroups[$cgKey]->container);
			for ($i=0; $i<$count1; $i++)
				for ($j=0; $j<$count2; $j++)
					if (!isset($columnGroups[$cgKey]->extra['parent']->extra['ids'][$cgKeys1[$i]]->{$columnGroups[$cgKey]->container[$cgKeys2[$j]]}))
						$columnGroups[$cgKey]->extra['parent']->extra['ids'][$cgKeys1[$i]]->{$columnGroups[$cgKey]->container[$cgKeys2[$j]]} = array();
		}

		foreach ($cgKeys as $cgKey)
			if ($columnGroups[$cgKey]->extra['parent'] == null)
				return array_values($columnGroups[$cgKey]->extra['ids']);
	}
