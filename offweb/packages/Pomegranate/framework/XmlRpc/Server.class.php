 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
class Server extends \Pomegranate\framework\RpcServer\RpcAbstract
{
	/**
	 * Character encoding
	 * @var string
	...
	/**
	 * Class to use for responses; defaults to {@link \Pomegranate\frmaework\XmlRpc\Response}
	 * @var string
	 */
	protected $_responseClass = '\Pomegranate\frmaework\XmlRpc\Response';
	...
	public function __call($method, $params)
	{
		...
		if (!method_exists($system, $method)) {
			$this->fault("Unknown instance method called on server: $method", RpcServer\Error::ERROR_INVALID_METHOD);
			return;
		}
	...
	public function addFunction($function, $namespace = '')
	{
		if (!is_string($function) && !is_array($function)) {
			$this->fault("Unable to attach function; invalid: $function", RpcServer\Error::ERROR_INVALID_METHOD);
			return;
		}
	...
	public function setClass($class, $namespace = '', $argv = null)
	{
		if (is_string($class) && !class_exists($class)) {
			$this->fault("Invalid method class: $class", RpcServer\Error::ERROR_INVALID_CLASS);
			return;
		}
	 ...
	 *
	 * @param \Pomegranate\framework\RpcServer\Request $request Optional
	 * @return \Pomegranate\framework\RpcServer\Response|\Zend_XmlRpc_Fault
	 */
	public function handle($request = false)
	{
		if ((false !== $request) && (!$request instanceof Request)) {
			require_once 'Zend/Json/Server/Exception.php';
			$this->fault('Invalid request type provided; cannot handle', RpcServer\Error::ERROR_INVALID_REQUEST);
			return;
		}
		else if ($request) {
		...
		// Get response
		$this->getResponse()->setEncoding($this->getEncoding());
	...
	public function loadFunctions($definition)
	{
		if (!is_array($definition) && (!$definition instanceof \Zend_Server_Definition)) {
			...
			$this->fault('Unable to load server definition; must be an array or Zend_Server_Definition, received ' . $type, RpcServer\Error::ERROR_INVALID_METHOD);
			return;
		}
	 ...
	 *
	 * @param string|\Pomegranate\framework\RpcServer\Request $request
	 * @return \Pomegranate\framework\RpcServer
	 * @throws \Zend_XmlRpc_Server_Exception on invalid request class or object
	 */
	public function setRequest(\Pomegranate\framework\RpcServer\Request $request)
	{
		if (is_string($request) && class_exists($request)) {
			$request = new $request();
			if (!$request instanceof \Pomegranate\framework\RpcServer\Request) {
				$this->fault('Invalid request class', RpcServer\Error::ERROR_INVALID_REQUEST);
				return $this;
			}
			$request->setEncoding($this->getEncoding());
		}
		else if (!$request instanceof \Pomegranate\framework\RpcServer\Request) {
			$this->fault('Invalid request object', RpcServer\Error::ERROR_INVALID_REQUEST);
			return $this;
		}
	 ...
	 *
	 * @return null|\Pomegranate\framework\RpcServer\Request
	 */
	...
	public function setResponseClass($class)
	{
		...
		|| ($c = new \ReflectionClass($class) and !$c->isSubclassOf('\Pomegranate\framework\RpcServer\Response'))) {
			$this->fault('Invalid response class', RpcServer\Error::ERROR_INVALID_REQUEST);
			return false;
		}
	 ...
	 * Get response object
	 *
	 * @return \Pomegranate\framework\RpcServer\Response
	 */
	public function getResponse()
	{
		if (null === ($response = $this->_response)) {
			$this->setResponse(new \Pomegranate\framework\XmlRpc\Response());
		}
	...
	protected function _handle()
	{
		...
		if (!$request->isFault() && (null === $request->getMethod())) {
			$this->fault('Invalid Method', RpcServer\Error::ERROR_INVALID_METHOD);
			return;
		}

		if ($request->isFault()) {
			$errors = $request->getFault();
			foreach ($errors as $e) {
				$this->fault($e->getMessage(), $e->getCode(), $e->getData());
			}
			return;
		}
		...
		if (!$this->_table->hasMethod($method)) {
			$this->fault('Method not found', RpcServer\Error::ERROR_INVALID_METHOD);
			return;
		}
		...
		if (!$matched) {
			$this->fault('Calling parameters do not match signature', RpcServer\Error::ERROR_INVALID_PARAMS);
			return;
		}

		try {
			$result = $this->_dispatch($info, $params);
		}
		catch (\Exception $e) {
