 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
class Server extends RpcServer\RpcAbstract
{
	/**#@+
	 * Version Constants
	 */
	 ...
	 * @return \Pomegranate\framework\RpcServer\Abstract
	 */
	public function addFunction($function, $namespace = '')
	{
		if (!is_string($function) && (!is_array($function) || (2 > count($function)))) {
			require_once 'Zend/Json/Server/Exception.php';
			$this->fault('Unable to attach function; invalid', RpcServer\Error::ERROR_INVALID_METHOD);
			return $this;
		}

		if (!is_callable($function)) {
			require_once 'Zend/Json/Server/Exception.php';
			$this->fault('Unable to attach function; does not exist', RpcServer\Error::ERROR_INVALID_METHOD);
			return $this;
		}
	 ...
	 * Handle request
	 *
	 * @param  \Pomegranate\framework\RpcServer\Request $request
	 * @return void|\Pomegranate\framework\RpcServer\Response
	 */
	...
	public function loadFunctions($definition)
	{
		if (!is_array($definition) && (!$definition instanceof \Zend_Server_Definition)) {
			require_once 'Zend/Json/Server/Exception.php';
			$this->fault('Invalid definition provided to loadFunctions()', RpcServer\Error::ERROR_INVALID_METHOD);
			return;
		}
	 ...
	 *
	 * @return \Pomegranate\framework\RpcServer\Request
	 */
	...
	public function getResponse()
	{
		if (null === ($response = $this->_response)) {
			$this->setResponse(new \Pomegranate\framework\Json\Response());
		}
	...
	protected function _getParams(\Zend_Server_Method_Definition $method)
	{
		...
		foreach ($method->getPrototypes() as $prototype) {
			foreach ($prototype->getParameterObjects() as $key => $parameter) {
				...
				else if (in_array($newType, $params[$key]['type'])) {
					continue;
				}
	 ...
	 * Set response state
	 *
	 * @return \Pomegranate\framework\Json\Response
	 */
	...
	protected function _handle()
	{
		...
		if (is_string(key($params))) {
			...
			foreach ($reflection->getParameters() as $refParam) {
				...
				else if ($refParam->isOptional()) {
					$orderedParams[$refParam->getName()] = null;
				}
				else {
					$this->fault('Missing required parameter: ' . $refParam->getName(), RpcServer\Error::ERROR_INVALID_PARAMS);
					return;
				}
			}
