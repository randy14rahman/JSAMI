...
var Pomegranate = window.Pomegranate = Pome = window.Pome = new (function() {
	...
	__anar.package = function(input) {
		...
		if (input.id && __anar.packages[input.id]) {
			...
			Pomegranate.create(undefined, input.class_path, [this], function() {
	...
	__anar.download = function(input) {
		...
		//window.open("./json-req:resource" + classPath + "?req=" + encodeURIComponent(jQuery.toJSON(params)));
		window.location.href = "./json-req:resource" + classPath + "?req=" + encodeURIComponent(jQuery.toJSON(params));
